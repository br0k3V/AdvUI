...
-- Frames and stuff are down there. Main functions are line 916 and above
...
-- // NEWKEYSYSTEM, NEWNOTIFICATION, NEWUIWINDOW, STARTUP
-- Gui to Lua
-- Version: 3.2
...
local function YKQFEK_fake_script() -- PlayerPFP.LocalScript 
...
coroutine.wrap(YKQFEK_fake_script)()
local function REBL_fake_script() -- ConfirmKey.CKey 
...
coroutine.wrap(REBL_fake_script)()
...
local function NWKO_fake_script() -- Startup.LocalScript 
...
coroutine.wrap(NWKO_fake_script)()
...
function br0k3UI.CreateButton(name, callbackResponse)
	-- max of 2 buttons until BETA

	local BtnTwo = script.Parent.Parent.NewUIWindow.PageONE.Button2
	local BtnOne = script.Parent.Parent.NewUIWindow.PageONE.Button1
	local OneEnable = false
	local TwoEnable = false

	if OneEnable == false then
		OneEnable = true
		BtnOne.Visible = true
		BtnOne.TextButton.MouseButton1Click:Connect(function()
			callbackResponse()
		end)
	end

	if TwoEnable == false then
		TwoEnable = true
		BtnTwo.Visible = true
		BtnTwo.TextButton.MouseButton1Click:Connect(function()
			callbackResponse()
		end)
	end
